...
user_file = input('Enter your file: ')
...
def treetops(user_file):
    ...
    for row in range(1,len(init_arr)-1):
        ...
        for i in range(1,len(temp)-1):
            ...
            if all(x < temp[i] for x in left) or all(x < temp[i] for x in right):
                row_total += 1
                temp2.append((temp[i],temp.index(temp[i])))
    ...
    for i in row_vis_list:
        for j in i:
            ...
            top = col[:j[1]]
            bottom = col[j[1]+1:]
            if all(x < col[j[0]] for x in top) or all(x < col[j[0]] for x in bottom):
                cross_total += 1
